...
#SSH
  echo -e '\n```mathematica' > "/tmp/TG_BOT.md"
  echo -e "\n[+] SSH:\n" >> "/tmp/TG_BOT.md"
  echo -e '!#Windows Powershell' >> "/tmp/TG_BOT.md"
  echo -e "\$env:SSHKEY=\"$SSHKEY\"" >> "/tmp/TG_BOT.md"
  echo -e "\$env:SSHKEY = \"\$(\$env:SSHKEY | Out-File \"\$env:TEMP\ssh_key.pem\" -Force; \"\$env:TEMP\ssh_key.pem\")\"" >> "/tmp/TG_BOT.md"
  echo -e "\n[+] Connect:" >> "/tmp/TG_BOT.md"
  echo -e "ssh -i "\"\$env:SSHKEY\"" \"runner@$NGROK_TCP_HOST\" -p \"$NGROK_TCP_PORT\" -o \"StrictHostKeyChecking=no\"" >> "/tmp/TG_BOT.md"
  echo -e "\n\n" >> "/tmp/TG_BOT.md"
  echo -e '!#Linux' >> "/tmp/TG_BOT.md"
  echo -e 'export SSHKEY="$(mktemp)"' >> "/tmp/TG_BOT.md"
  echo -e "echo \"$SSHKEY\" > "\"\$SSHKEY\""" >> "/tmp/TG_BOT.md"
  echo -e "\n[+] Connect:\n" >> "/tmp/TG_BOT.md"
  echo -e "ssh -i "\"\$SSHKEY\"" \"runner@$NGROK_TCP_HOST\" -p \"$NGROK_TCP_PORT\" -o \"StrictHostKeyChecking=no\"" >> "/tmp/TG_BOT.md"
  echo -e '```' >> "/tmp/TG_BOT.md"
#Send Notif
 #apprise "tgram://$TG_BOT_AUTH/?topic=$LOONIX_TOPIC_ID&format=markdown" -vv -b "$(cat "/tmp/TG_BOT.md")"
 apprise "tgram://$TG_BOT_AUTH/?format=markdown" -vv -b "$(cat "/tmp/TG_BOT.md")"
#----------------------------------------------------------------------------#
