...
#Get Instance_ID
 CONTABO_INSTANCE_ID="$(cntb get instances --oauth2-clientid="$(echo "${CONTABO_CLIENT_ID}" | sed -e 's/"//g' -e "s/'//g" | tr -d '[:space:]')" --oauth2-client-secret="$(echo "${CONTABO_CLIENT_SECRET}" | sed -e 's/"//g' -e "s/'//g" | tr -d '[:space:]')" --oauth2-user="$(echo "${CONTABO_API_USERNAME}" | sed -e 's/"//g' -e "s/'//g" | tr -d '[:space:]')" --oauth2-password="$(echo "${CONTABO_API_PASSWORD}" | sed -e 's/"//g' -e "s/'//g" | tr -d '[:space:]')" --debug="info" --output="json" | jq --arg HN "$HOST_NUMBER" '.[] | select(.vHostNumber == ($HN | tonumber))' | jq -r '.instanceId')" && export CONTABO_INSTANCE_ID="${CONTABO_INSTANCE_ID}"
